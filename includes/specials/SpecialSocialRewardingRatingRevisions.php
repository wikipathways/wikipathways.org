...
/**
 * Special page for displaying list of rated revisions.
 */
class SpecialSocialRewardingRatingRevisions extends PageQueryPage {
	 ...
	 */
	function getName() {
		return "SocialRewardingRatingRevisions";
	}
	...
	function getSQL() {
		...
		$sql  = "SELECT
				'Social Rewarding: Rating of Revisions' AS type,
				page_namespace AS namespace,
				page_title AS title,
				ROUND((points / count), " . $SocialRewarding["reward"]["round"] . ") AS value,
			...
			FROM
				...
				$page
			WHERE
				" . $dbr->tableName($SocialRewarding["DB"]["rating"]) . ".rev_id = $revision.rev_id AND
		...
		$output = "
			<form action=" . $action . " method=get>
				...
				<select name=sr_orderby>
					...
					<option value=value"; if ($_GET["sr_orderby"]=="value" || !$_GET["sr_orderby"]) $output.=" selected"; $output.=">score</option>
 ...
 * @access public
 */
function wfSpecialSocialRewardingRatingRevisions() {
	list($limit,$offset) = wfCheckLimits();
	$site = new SpecialSocialRewardingRatingRevisions();
