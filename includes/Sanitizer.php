...
class Sanitizer {
	...
	static function checkCss( $value ) {
		...
		// ... and continue checks
		//DEPRECARTED// $stripped = preg_replace( '!\\\\([0-9A-Fa-f]{1,6})[ \\n\\r\\t\\f]?!e',
		//	'codepointToUtf8(hexdec("$1"))', $stripped );
		$stripped = preg_replace_callback('!\\\\([0-9A-Fa-f]{1,6})[ \\n\\r\\t\\f]?!',
			create_function('$matches','return codepointToUtf8(hexdec($matches[1]));'), $stripped);
	...
	static function cleanUrl( $url, $hostname=true ) {
		...
		//DEPRECATED// $url = preg_replace( '/[\][<>"\\x00-\\x20\\x7F]/e', "urlencode('\\0')", $url );
		$url = preg_replace_callback('/[\][<>"\\x00-\\x20\\x7F]/',
			create_function ('$matches','return urlencode($matches[0]);'), $url);
