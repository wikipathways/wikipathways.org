...
/**
 * Special page for displaying list of revisions with amount of
 * references.
 */
class SpecialSocialRewardingReferencesRevisions extends PageQueryPage {
	 ...
	 */
	function getName() {
		return "SocialRewardingReferencesRevisions";
	}
	...
	function getSQL() {
		...
		$sql  = "SELECT
				'Social Rewarding: Amount of References of Revisions' AS type,
				page_namespace AS namespace,
				page_title AS title,
			";
			if ($_GET["sr_weight"]) {
		$sql .= "	ROUND(((size / count) + (link / count) + self_link), " . $SocialRewarding["reward"]["round"] . ") AS value, ";
			} else {
		$sql .= "	ROUND(size + link + self_link, " . $SocialRewarding["reward"]["round"] . ") AS value, ";
		}
	...
	function formatResult($skin, $result) {
		...
		$output = $skin->makeLink($addUser.$result->title, "[R" . $result->rev_id . "] $addUser$titleText", "oldid=" . $result->rev_id, " (" . $result_value . " points) $author$comma$date");
 ...
 * @access public
 */
function wfSpecialSocialRewardingReferencesRevisions() {
	list($limit,$offset) = wfCheckLimits();
	$site = new SpecialSocialRewardingReferencesRevisions();
